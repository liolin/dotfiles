...
#+begin_src sh 
  if status is-login
     set -gx PATH "$HOME/.cargo/bin" "$HOME/.dotnet/tools" "$HOME/.local/bin" $PATH

     if test -d "/var/lib/flatpak/exports/share/applications"
	set -gx XDG_DATA_DIRS "/var/lib/flatpak/exports/share/applications" $XDG_DATA_DIRS
     end

     if test -d "$HOME/.local/share/flatpak/exports/share/applications"
	set -gx XDG_DATA_DIRS "$HOME/.local/share/flatpak/exports/share/applications" $XDG_DATA_DIRS
     end 
     ...
     test -f "$HOME/.nix-profile/etc/profile.d/nix.sh" && fenv source "$HOME/.nix-profile/etc/profile.d/nix.sh"
     test -f "$HOME/.nix-profile/etc/profile.d/hm-session-vars.sh" && fenv source "$HOME/.nix-profile/etc/profile.d/hm-session-vars.sh"

     set -gx ALTERNATE_EDITOR ""
     set -gx EDITOR "emacsclient -t"                  # $EDITOR opens in terminal
     set -gx VISUAL "emacsclient -t -a emacs"         # $VISUAL opens in GUI mode
     set -gx DOTNET_CLI_TELEMETRY_OPTOUT 1
     set -gx LEDGER_FILE "~/finance/(date +%Y).journal.gpg"
